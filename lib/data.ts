...
async function fetchWithCache<T>(
    ...
    try {
        ...
        if (!contentType || !contentType.includes("application/json")) {
            console.error(
                `Expected JSON but got ${contentType} for URL: ${url}`,
            );
            // Don't throw error, return empty data instead
            return { data: [], success: false } as T;
        }
...
export const getArticlesByAuthor = (authorSlug: string) => {
    return cachedFetch<{ data: Article[]; meta: any }>(
        `${BACK_END}/articles?author=${authorSlug}`,
...
export const getNavbar = async (): Promise<{ data: NavbarItem[] }> => {
    const response = await fetch(`${BACK_END}/navbar?sort=idx`);
    if (!response.ok) {
...
export const getFooterData = async (): Promise<{ data: FooterApiData[] }> => {
    const response = await fetch(`${BACK_END}/footer`);
    if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
    }
    return response.json();
...
export const searchArticles = (
    ...
    return cachedFetch<{ data: Article[]; meta: any }>(
        `${BACK_END}/articles/search?${params}`,
...
export const getFeaturedArticles = cachedFetch<{ data: Article[] }>(
    `${BACK_END}/articles?featured=true`,
    "featured-articles",
);

// Recent articles
export const getRecentArticles = cachedFetch<{ data: Article[] }>(
    `${BACK_END}/articles?sort=publishedAt&order=desc&limit=10`,
    "recent-articles",
);
...
export const getRecentArticlesClient = (): Promise<{ data: Article[] }> =>
    fetch("/api/articles?sort=publishedAt&order=desc&limit=10").then((res) =>
        res.json(),
    );

// Popular articles
export const getPopularArticles = cachedFetch<{ data: Article[] }>(
    `${BACK_END}/articles?sort=views&order=desc&limit=10`,
    "popular-articles",
);
...
export async function getCategoryPageData(categorySlug: string) {
    try {
        ...
        const transformedArticles = (articlesResult?.data || [])
            .map((article: any) => {
                try {
            ...
            .filter(
                (article: Article | null): article is Article =>
                    article !== null,
            );
